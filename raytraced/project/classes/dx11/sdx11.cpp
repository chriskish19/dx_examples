...
HRESULT core::create_device(device_description* dd) {
...
core::device_description::~device_description()
{
	if (pAdapter != nullptr) {
		pAdapter->Release();
	}
...
ID3D11Texture2D* core::cbb(IDXGISwapChain* pSwapChain) {
...
ID3D11RenderTargetView* core::crtv(ID3D11Device* pDevice, ID3D11Texture2D* pBackBuffer) {
	// render target view
...
ID3D11Texture2D* core::cdsb(const D3D11_TEXTURE2D_DESC* p_dsd, ID3D11Device* p_device) {
...
ID3D11DepthStencilView* core::cdsv(const D3D11_DEPTH_STENCIL_VIEW_DESC* p_dsvd, ID3D11Texture2D* p_dsb, ID3D11Device* p_device) {
	// depth stencil view
